...
exports.partiallyEvalExpression = exports.evalConstantExpression = exports.evalBinaryOp = exports.evalUnaryOp = void 0;
...
function fullyEvalUnaryOp(op, operand, source, ctx) {
...
function fullyEvalBinaryOp(op, left, right, source, ctx) {
...
function partiallyEvalBinaryOp(op, left, right, source, ctx) {
    ...
    if ((0, ast_1.isValue)(leftOperand) && (0, ast_1.isValue)(rightOperand)) {
        ...
        const result = evalBinaryOp(op, valueLeftOperand, valueRightOperand, leftOperand.loc, rightOperand.loc, source);
        // Wrap the value into a Tree to continue simplifications
        return (0, util_1.makeValueExpression)(result);
    }
    else {
...
function fullyEvalConditional(condition, thenBranch, elseBranch, ctx) {
    // here we rely on the typechecker that both branches have the same type
    const valCond = ensureBoolean(evalConstantExpression(condition, ctx), condition.loc);
    if (valCond) {
...
// In the process of writing a partiallyEval version of this
// function for the partial evaluator
function fullyEvalStructInstance(structTypeId, structFields, ctx) {
    const structTy = (0, resolveDescriptors_1.getType)(ctx, structTypeId);
    ...
    const resultWithDefaultFields = structTy.fields.reduce((resObj, field) => {
        ...
        return resObj;
    }, { $tactStruct: (0, ast_1.idText)(structTypeId) });
    // this will override default fields set above
    return structFields.reduce((resObj, fieldWithInit) => {
        resObj[(0, ast_1.idText)(fieldWithInit.field)] = evalConstantExpression(fieldWithInit.initializer, ctx);
        return resObj;
    }, resultWithDefaultFields);
}
// In the process of writing a partiallyEval version of this
// function for the partial evaluator
function fullyEvalFieldAccess(structExpr, fieldId, source, ctx) {
    // special case for contract/trait constant accesses via `self.constant`
    if (structExpr.kind === "id" && (0, ast_1.isSelfId)(structExpr)) {
        ...
        if (selfTypeRef.kind == "ref") {
            ...
            const foundContractConst = contractTypeDescription.constants.find((constId) => (0, ast_1.eqNames)(fieldId, constId.name));
            if (foundContractConst === undefined) {
                // not a constant, e.g. `self.storageVariable`
                throwNonFatalErrorConstEval(`cannot a evaluate non-constant self field access`, structExpr.loc);
            }
            if (foundContractConst.value !== undefined) {
                return foundContractConst.value;
            }
            else {
                throwErrorConstEval(`cannot evaluate declared contract/trait constant ${(0, errors_1.idTextErr)(fieldId)} as it does not have a body`, fieldId.loc);
            }
        }
    }
    ...
    if (valStruct == null ||
        ...
        throwErrorConstEval(`constant struct expected, but got ${(0, types_1.showValue)(valStruct)}`, structExpr.loc);
    }
    if ((0, ast_1.idText)(fieldId) in valStruct) {
        return valStruct[(0, ast_1.idText)(fieldId)];
    }
    else {
        // this cannot happen in a well-typed program
        throwErrorConstEval(`struct field ${(0, errors_1.idTextErr)(fieldId)} is missing`, structExpr.loc);
    }
}
// In the process of writing a partiallyEval version of this
// function for the partial evaluator
function fullyEvalMethod(methodName, object, args, source, ctx) {
    switch ((0, ast_1.idText)(methodName)) {
        case "asComment": {
            ensureMethodArity(0, args, source);
            const comment = ensureString(evalConstantExpression(object, ctx), object.loc);
            return new types_1.CommentValue(comment);
        }
        default:
            throwNonFatalErrorConstEval(`calls of ${(0, errors_1.idTextErr)(methodName)} are not supported at this moment`, source);
    }
}
// In the process of writing a partiallyEval version of this
// function for the partial evaluator
function fullyEvalBuiltins(builtinName, args, source, ctx) {
    switch ((0, ast_1.idText)(builtinName)) {
        case "ton": {
            ensureFunArity(1, args, source);
            const tons = ensureString(evalConstantExpression(args[0], ctx), args[0].loc);
            try {
                return ensureInt(BigInt((0, core_1.toNano)(tons).toString(10)), source);
            }
            catch (e) {
                if (e instanceof Error && e.message === "Invalid number") {
                    throwErrorConstEval(`invalid ${(0, errors_1.idTextErr)(builtinName)} argument`, source);
                }
                throw e;
            }
        }
        case "pow": {
            ...
            const valBase = ensureInt(evalConstantExpression(args[0], ctx), args[0].loc);
            const valExp = ensureInt(evalConstantExpression(args[1], ctx), args[1].loc);
            if (valExp < 0n) {
                throwErrorConstEval(`${(0, errors_1.idTextErr)(builtinName)} builtin called with negative exponent ${valExp}`, source);
            }
            try {
        ...
        case "pow2": {
            ensureFunArity(1, args, source);
            const valExponent = ensureInt(evalConstantExpression(args[0], ctx), args[0].loc);
            if (valExponent < 0n) {
                throwErrorConstEval(`${(0, errors_1.idTextErr)(builtinName)} builtin called with negative exponent ${valExponent}`, source);
            }
            try {
        ...
        case "sha256": {
            ensureFunArity(1, args, source);
            const str = ensureString(evalConstantExpression(args[0], ctx), args[0].loc);
        ...
        case "newAddress": {
            ensureFunArity(2, args, source);
            const wc = ensureInt(evalConstantExpression(args[0], ctx), args[0].loc);
            const addr = Buffer.from(ensureInt(evalConstantExpression(args[1], ctx), args[1].loc)
                .toString(16)
                .padStart(64, "0"), "hex");
            if (wc !== 0n && wc !== -1n) {
            ...
            if (!(0, features_1.enabledMasterchain)(ctx) && wc !== 0n) {
                throwErrorConstEval(`${wc}:${addr.toString("hex")} address is from masterchain which is not enabled for this contract`, source);
            }
            return new core_1.Address(Number(wc), addr);
        }
        default:
            throwNonFatalErrorConstEval(`unsupported builtin ${(0, errors_1.idTextErr)(builtinName)}`, source);
    }
}
...
function lookupName(ast, ctx) {
    if ((0, resolveDescriptors_1.hasStaticConstant)(ctx, ast.text)) {
        const constant = (0, resolveDescriptors_1.getStaticConstant)(ctx, ast.text);
...
function evalConstantExpression(ast, ctx) {
    switch (ast.kind) {
        ...
        case "conditional":
            return fullyEvalConditional(ast.condition, ast.thenBranch, ast.elseBranch, ctx);
...
function partiallyEvalExpression(ast, ctx) {
    switch (ast.kind) {
        case "id":
            try {
                return (0, util_1.makeValueExpression)(lookupName(ast, ctx));
            }
            catch (e) {
        ...
        case "method_call":
            ...
            return (0, util_1.makeValueExpression)(fullyEvalMethod(ast.method, ast.self, ast.args, ast.loc, ctx));
        case "init_of":
            throwNonFatalErrorConstEval("initOf is not supported at this moment", ast.loc);
            break;
        case "null":
        ...
        case "boolean":
            return ast;
        case "number":
            return (0, util_1.makeValueExpression)(ensureInt(ast.value, ast.loc));
        case "string":
            return (0, util_1.makeValueExpression)(ensureString(interpretEscapeSequences(ast.value, ast.loc), ast.loc));
        case "op_unary":
        ...
        case "conditional":
            // Does not partially evaluate at the moment. Will attempt to fully evaluate
            return (0, util_1.makeValueExpression)(fullyEvalConditional(ast.condition, ast.thenBranch, ast.elseBranch, ctx));
        case "struct_instance":
            // Does not partially evaluate at the moment. Will attempt to fully evaluate
            return (0, util_1.makeValueExpression)(fullyEvalStructInstance(ast.type, ast.args, ctx));
        case "field_access":
            // Does not partially evaluate at the moment. Will attempt to fully evaluate
            return (0, util_1.makeValueExpression)(fullyEvalFieldAccess(ast.aggregate, ast.field, ast.loc, ctx));
        case "static_call":
            // Does not partially evaluate at the moment. Will attempt to fully evaluate
            return (0, util_1.makeValueExpression)(fullyEvalBuiltins(ast.function, ast.args, ast.loc, ctx));
    }
}
