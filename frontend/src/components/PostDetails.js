...
const PostDetails = () => {
    ...
    const renderComments = (comments, depth = 0) => comments.map((comment, index) => (
        <div key={`${comment._id}-${depth}-${index}`} className={`comment bg-gray-1000 p-4 mb-4 rounded-lg text-left ml-${depth * 4}`}>
            ...
            {replyingTo === comment._id && <CommentForm postId={postId} parentCommentId={comment._id} onCommentSubmit={handleNewComment} />}
